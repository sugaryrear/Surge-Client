...
public class Rasterizer2D extends Cacheable implements RSRasterizer2D {
    ...
    public static void renderPolygon(Shape poly, Color color) {
        Graphics2D graphics = Rasterizer2D.createGraphics(true);
    ...
    public static Graphics2D createGraphics(boolean renderingHints) {
        Graphics2D g2d = createGraphics(Rasterizer2D.pixels, Rasterizer2D.width, Rasterizer2D.height);
        if (renderingHints) {
...
//draw_rect_outline
    ...
    public static void drawTransparentVerticalLine(int xPosition, int yPosition, int height, int rgbColour, int opacity) {
        if (xPosition < topY  || xPosition >= bottomY) {
            return;
        }
        if (yPosition < leftX) {
            height -= leftX - yPosition;
            yPosition = leftX;
        }
        if (yPosition + height >  bottomX) {
            height =  bottomX - yPosition;
        }
        final int transparency = 256 - opacity;
        final int red = (rgbColour >> 16 & 0xff) * opacity;
        final int green = (rgbColour >> 8 & 0xff) * opacity;
     ...
     */
    public static void draw_filled_rect(int leftX, int topY, int width, int height, int rgbColour, int opacity) {//drawtransparentbox
        if (leftX < Rasterizer2D.leftX) {
            width -= Rasterizer2D.leftX - leftX;
            leftX = Rasterizer2D.leftX;
        }
