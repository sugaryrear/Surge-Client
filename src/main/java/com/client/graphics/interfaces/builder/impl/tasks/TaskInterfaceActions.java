...
//what did this code look like originally
    public void setOpen(TaskEntryType type) {
        TaskInterfaceSub sub = inter.byType(type);
        inter.state = sub.getType();
      //  System.out.println("type:"+sub.getType()+" sibint: "+sub.getInterfaceId());
        if(type == TaskEntryType.DIARIES)
        RSInterface.get(29465).children[0] = sub.getInterfaceId();//change this look create new interface then put this shit into it !
        else

        RSInterface.get(32561).children[0] = sub.getInterfaceId();//change this look create new interface then put this shit into it !
     //   System.out.println("int: "+sub.getInterfaceId());
    ...
    private void addEntries(TaskEntryType type, List<TaskEntry> entries) {
        ...
        for (TaskEntry entry : entries) {
            if (entry.isFiltered(filter)) {//check if its achievement diary
